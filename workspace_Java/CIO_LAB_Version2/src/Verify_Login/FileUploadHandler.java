	   ...
	   protected void service(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
	    ...
	    void processFormField(FileItem item,HttpServletRequest request,HttpServletResponse response) throws ServletException, IOException
	    {
	    	...
	    	System.out.println("Inside item.isFormField()-- True");
        	 ...
        	 // Setting temp file location to avoid sumulatenous clash of file upload b/w to users. New Upload path is C\temp\IP
        	
	    }
	    void processUploadedFile(FileItem item,HttpServletRequest request,HttpServletResponse response) throws Exception
	    {
	    	String Temp_File_Loc=null;
	    	String fname = null;
            String fsize = null;
            String ftype = null;
    		...
    		try
    		{
	    		...
	    		Temp_File_Loc=UPLOAD_DIRECTORY+"/"+HOST_IP_UPLOAD_PAGE;
	        	
	        	
	        	System.out.println("Upload Directory TEMP_FL_LOC="+Temp_File_Loc);
    			...
    			e.printStackTrace();
    			request.setAttribute("File_Upload_Status", "Cannot create FOLDER with Index to store file, Acess Denied"); 
    		...
    		// Cheking if temp directory creation is done or not.
    		...
    		if(!Temp_File_Loc.equals(null))
    		{
    			System.out.println("tempfile Loc="+Temp_File_Loc);
    			
    			String File_Path=Temp_File_Loc+"/"+fname;
    			...
    			System.out.println("File obejct created with ff ="+ff);
    			
    			System.out.println("Calling write on ff Start");
    			// Fileitem item has write(File file)
        		// A convenience method to write an uploaded item to disk.
                item.write(ff);
                
                System.out.println("Completeing write on ff Success");
    		...
    		else
    		{
    			request.setAttribute("File_Upload_Status", "Cannot create FOLDER with Index to store file, Acess Denied"); 
                request.setAttribute("IP_ADDR",HOST_IP_UPLOAD_PAGE);
                System.out.println("Folder Creation failed hence skipping file upload");
    			return;
    		}
